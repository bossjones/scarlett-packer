...
sudo apt-get install -y  a11y-profile-manager-indicator \
...
sudo apt-get update
sudo apt-get install -y gimp gimp-help-en oracle-java7-installer


# Install appropriate graphics and wlan drivers
lspci | grep VGA | grep -iq nvidia && sudo apt-get install -y  nvidia-current-updates
lspci | grep VGA | grep -q ATI && sudo apt-get install -y fglrx-updates
...
sudo tee -a /etc/lightdm/users.conf <<EOF
#
